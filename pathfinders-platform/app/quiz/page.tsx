...
import { GraduationCap, Star, Trophy, Target, Heart, Zap, ArrowRight, Sparkles, Bot } from "lucide-react"
...
export default function QuizPage() {
  ...
  const generatePlaceholderRecommendations = (profile: any[]): string => {
    const interests = profile.find((p) => p.category === "Academic Interest")?.answer || ""
    const workStyle = profile.find((p) => p.category === "Work Style")?.answer || ""
    ...
    if (interests.includes("Science and Technology")) {
      ...
      recommendations += "- **Software Engineering**: Build applications and systems\n"
      recommendations += "- **Data Science**: Analyze data to drive decisions\n"
      recommendations += "- **Cybersecurity**: Protect digital infrastructure\n"
      recommendations += "- **AI/ML Engineering**: Develop intelligent systems\n\n"

      recommendations += "### 📚 Recommended Learning Path\n"
      recommendations += "1. Master programming languages (Python, Java, JavaScript)\n"
      recommendations += "2. Build projects and contribute to open source\n"
      recommendations += "3. Pursue computer science or related engineering degree\n"
    ...
    if (interests.includes("Business and Economics")) {
      ...
      recommendations += "- **Management Consulting**: Solve complex business problems\n"
      recommendations += "- **Product Management**: Lead product development\n"
      recommendations += "- **Investment Banking**: Financial analysis and deals\n"
    ...
    if (purpose.includes("Educate and inspire")) {
      ...
      recommendations += "- **Corporate Training**: Develop professional skills\n"
      recommendations += "- **Educational Technology**: Create learning platforms\n"
      recommendations += "- **Academic Research**: Advance knowledge in your field\n\n"
    }
    ...
    recommendations += "4. Develop relevant skills through courses and projects\n"
    recommendations += "5. Consider informational interviews with industry experts"
          ...
          >
            <Card className="bg-white border-[#e5e1dc] text-center shadow-lg">
              <CardHeader>
                ...
                <CardTitle className="text-3xl text-[#8b4513] mb-2">Congratulations!</CardTitle>
                <p className="text-[#a0826d] text-lg">You've completed the personality assessment</p>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="flex justify-center items-center space-x-8">
                  <div className="text-center">
                    <div className="text-3xl font-bold text-[#d4621a]">{totalPoints}</div>
                    <div className="text-sm text-[#a0826d]">Points Earned</div>
                  </div>
                  <div className="text-center">
                    <div className="text-3xl font-bold text-[#8b4513]">{questions.length}</div>
                    <div className="text-sm text-[#a0826d]">Questions Completed</div>
                  </div>
                </div>
                    ...
                    {isGeneratingRecommendations ? (
                      <div className="space-y-4">
                        ...
                        <div className="w-full bg-[#e5e1dc] rounded-full h-2">
                          <div className="bg-[#d4621a] h-2 rounded-full animate-pulse" style={{ width: "70%" }}></div>
                        </div>
                      </div>
                    ) : (
                      <>
                        <p className="text-[#a0826d] mb-6">
                          Ready to discover your perfect career path? Our AI will analyze your responses and provide
                  ...
                  >
                    <div className="bg-[#f5f3f0] border border-[#e5e1dc] rounded-lg p-6 text-left">
                      <div className="flex items-center mb-4">
                        ...
                        <h3 className="text-lg font-semibold text-[#8b4513]">AI Career Analysis</h3>
                      </div>
                      <div className="prose prose-sm max-w-none text-[#8b4513]">
                        {aiRecommendations.split("\n").map((line, index) => {
                          if (line.startsWith("##")) {
                            return (
                              <h2 key={index} className="text-xl font-bold text-[#8b4513] mt-4 mb-2">
                                {line.replace("##", "").trim()}
                              </h2>
                            )
                          }
                          if (line.startsWith("###")) {
                            return (
                              <h3 key={index} className="text-lg font-semibold text-[#d4621a] mt-3 mb-2">
                                {line.replace("###", "").trim()}
                              </h3>
                            )
                          }
                    ...
                    <Button onClick={handleContinue} className="w-full bg-[#d4621a] hover:bg-[#b8541a] text-white">
      ...
      <div className="flex-1 p-4">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center">
            ...
            <span className="text-2xl font-bold text-[#8b4513]">PathFinders</span>
          </div>
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center mb-2">
            <span className="text-sm font-medium text-[#8b4513]">Progress</span>
            <span className="text-sm text-[#a0826d]">{Math.round(progress)}% Complete</span>
          </div>
                  ...
                  {questions[currentQuestion].options.map((option, index) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="text-[#8b4513] group-hover:text-[#d4621a] transition-colors">{option}</span>
